...
if (player getVariable["inCombat",false]) exitWith {dayz_actionInProgress = false; localize "str_epoch_player_43" call dayz_rollingMessages;};
...
if(_IsNearPlot == 0) then {
   ...
   // if(dayz_characterID == _ownerID) then {  //Keep ownership	
    ...
    if(_allowed) then {  //Keep ownership 
        ...
        if(!_isPole) then {  // disallow building plot
            _canBuildOnPlot = true;
        } else {
...
_hasrequireditem = true;
{
    ...
    if(!_hastoolweapon) exitWith { dayz_actionInProgress = false; _hasrequireditem = false; _missing = getText (configFile >> "cfgWeapons" >> _x >> "displayName"); };
} count _require;
//### BEGIN MODIFIED CODE player_deploy
    ...
    if(!_cancel) then {
        ...
        if (_proceed) then {
            ...
            //###BEGIN MODIFIED CODE
				...
				{
					_x action ["Eject", _tmpbuilt];
				} forEach crew _tmpbuilt;		
