...
DZE_AntiWallLimit = 1; // Number of activations before player_antiWall kills player for glitching attempt. Lower is stricter, but may result in false positives.
...
DZE_NoVehicleExplosions = true; //Disable vehicle explosions to prevent damage to objects by ramming. Doesn't work with amphibious pook which should not be used due to FPS issues.
...
DZE_SafeZoneNoBuildDistance = 300; // Distance from safe zones listed above to disallow building near.
...
DZE_NoBuildNearDistance = 300; // Distance from blacklisted objects to disallow building near.
...
DZE_NutritionDivisor = [0.1, 0.1, 0.1, 0.1]; //array of DIVISORS that regulate the rate of [calories, thirst, hunger, temperature] use when "working" (keep in mind that temperature raises with actions) - min values 0.1
...
// Dynamic Vehicles
DynamicVehicleDamageLow = 50; // Min damage random vehicles can spawn with
DynamicVehicleDamageHigh = 75; // Max damage random vehicles can spawn with
DynamicVehicleFuelLow = 5; // Min fuel random vehicles can spawn with
DynamicVehicleFuelHigh = 25; // Max fuel random vehicles can spawn with
MaxAmmoBoxes = 4; // Max number of random Supply_Crate_DZE filled with vehicle ammo to spawn around the map
...
DZE_HaloSpawnHeight = 1000; // This is the altitude fresh spawn players start at when HALO spawn is enabled.
...
DZE_serverLogTrades = false; // Log trades to server RPT (sent with publicVariableServer on every trade)
...
DZE_GemWorthArray = []; // Array of gem prices, only works with config traders. Set DZE_GemWorthArray=[]; to disable return change in gems.
...
Z_VehicleDistance = 70; // Max distance a vehicle can be sold or accessed from at a trader.
...
DZE_buildMaxHeightDistance = 50; // Max distance player can raise or lower object from start position when building.
...
DZE_doorManagementAdmins = ["76561197997835618"]; //Array of admin PlayerUIDs. UIDs in this list are able to access every door's management menu and open it.
